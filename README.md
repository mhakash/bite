...
- **Backend**: build a static binary (`go build ./cmd/server`) and run it
  behind nginx/Caddy on your server. Set `JWT_SECRET`, `CORS_ORIGINS` (your
  frontend's real domain), and `COOKIE_SAMESITE=none` (cross-domain cookies
  require HTTPS + `SameSite=None`).
- **Frontend**: point Vercel (or similar) at `frontend/`, set `VITE_API_URL`
  to your backend's public HTTPS URL.
